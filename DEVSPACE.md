...
- The first turn on a devspace thread runs `ds add` into `<base-dir>/devspaces/<repo>/<thread-id>`. If bootstrap fails before the first message is recorded, sending from the zero-message server thread retries checkout preparation. The toolbar selects `trunk()` by default, a bookmark, a `workspace@` head, or a free-text change ID/revset (resolved server-side by `ds add -r`; refs served by the `devspace.refs.list` WS method). New change mode creates a fresh child; edit change mode passes `--edit` to work directly on a mutable selected revision.
...
Tracked in `docs/project/todo.md` (devspace items), including the unarchive dangling-checkout guard and project-recognition gaps.
